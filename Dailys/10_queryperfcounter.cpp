...
int CALLBACK WinMain(
    ...
    { // Class has to be registered ? Function retruns an "ATOM" type thing but it isnt needed?
        ...
        {   
            ...
            perf_timer input_test_timer;  
            input_test_timer.id = 0;           

            perf_timer draw_test_timer;  
            draw_test_timer.id = 1;

            perf_timer sound_test_timer;  
            sound_test_timer.id = 2;
            ...
            while(program_running)
            {
                ...
                perf_timer_start(&draw_test_timer);
                ...
                perf_timer_stop(&draw_test_timer);


                perf_timer_start(&sound_test_timer);

                //Sound output                
                sound_test(sound_buffer, kb_x_input, kb_y_input);  //Locks, writes and unlocks the buffer
                
                perf_timer_stop(&sound_test_timer);
