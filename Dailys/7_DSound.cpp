...
internal void load_dsound(HWND window, INT32 samples_per_second, INT32 audio_buffer_size){     
    ...
    if(dsound_library){
        ...
        if(p_direct_sound_create && SUCCEEDED(p_direct_sound_create(0, &p_direct_sound_object,0))){ //Alternative? :p_direct_sound_create && DirectSoundCreate(0,0,0)==DS_OK
            ...
            if(SUCCEEDED(p_direct_sound_object->SetCooperativeLevel(window, DSSCL_PRIORITY))){
                ...
                secondary_buffer_description.dwFlags = 0; //TODO: This doenst work
                ...
                if(SUCCEEDED(p_direct_sound_object->CreateSoundBuffer(&secondary_buffer_description, &p_secondary_buffer, 0))){
                    HRESULT second_error = p_secondary_buffer->SetFormat(&wave_format);
                    if(SUCCEEDED(second_error)){
...
int CALLBACK WinMain(
    ...
    { // Class has to be registered ? Function retruns an "ATOM" type thing but it isnt needed?
        ...
        {                                  
            ...
            load_dsound(window,44100,176400);      //Loads DSound TODO: replace with XAudio ! 44100 samples per s, 4 bytes per sample (2 bytes per channel)
...
DirectSoundCreate(
    LPCGUID lpGUID,         //audio device NULL = default
    LPDIRECTSOUND *ppDS,    //  Address of a pointer to a DirectSound object created in response to this function.
    LPUNKNOWN pUnkOuter      // has to be null
    );
