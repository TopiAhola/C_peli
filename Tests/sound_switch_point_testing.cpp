...
#include <C:\Users\valas\VSCProjects\C_peli\Build_environment\Build_daily_12\game_header.h>
...
internal void sound_test(game_soundbuffer * soundbuffer, uint32 samples_used, int x_input, int y_input){  
    ...
    local_static int32 freq = 100;
    local_static int32 target_freq = 100;
    ...
    uint32 target_period = (soundbuffer->sample_rate / target_freq);
    local_static int32 switch_point = 0;    
    uint32 relative_switch_point;
    
    
    int16 value;
    int16 * channel_p = soundbuffer->memory_p;
    //TODO: Better frequency switching or remove this
    if(freq == target_freq){
        ...
        //Find switch point
        for(
        uint32 sample = sample_counter; 
        sample < sample_counter + (soundbuffer->size / soundbuffer->bytes_per_sample); 
        sample++) 
        {
            if( ((sample % period) && (sample % target_period)) ){
                switch_point = sample;
                relative_switch_point = (switch_point % ( soundbuffer->size / soundbuffer->bytes_per_sample ));
                break;
            }

        }
        
        //Write freq till switch point
        for(int sample = 0; sample < relative_switch_point; sample++){
        ...
        //Write target freq from switch point        
        for(int sample = relative_switch_point; 
                sample < ( soundbuffer->size / soundbuffer->bytes_per_sample ); 
                sample++){
            
            //Sine wave function value
            float32 phase = sinf( (2.0f * pi32 * ( (float32)sample_counter) / ((float32)target_period) ) );
            value = (int16)(amplitude * phase) ;
        ...
        if(sample_counter < relative_switch_point) 
        freq = target_freq;
    }
